      ...
      for (int col = 0; col < num_cols; ++col) {
        ...
        for (int y = y_start; y < y_start + block_h && y < height; y++) {
          for (int x = x_start; x < x_start + block_w && x < width; x++) {
            ...
            float px_diff =
                CONVERT_TO_SHORTPTR(source->y_buffer)[y * source->y_stride + x]
        ...
        for (int y = y_start >> ss_y; y < y_end; y++) {
          ...
          for (int x = x_start >> ss_x; x < x_end; x++) {
            ...
            const float px_diff_u = (float)
                (CONVERT_TO_SHORTPTR(source->u_buffer)[src_px_index] -
                CONVERT_TO_SHORTPTR(recon->u_buffer)[recon_px_index]);
            const float px_diff_v = (float)
                (CONVERT_TO_SHORTPTR(source->v_buffer)[src_px_index] -
...
void av1_setup_butteraugli_source(AV1_COMP *cpi) {
  ...
  if (cm->seq_params->use_highbitdepth) {
    ...
    copy_img_highbd(resized_dst, cpi->source, width / resize_factor, height /
        resize_factor);
  } else {
    zero_img_lowbd(cpi->source);
    copy_img_lowbd(resized_dst, cpi->source, width / resize_factor, height /
        resize_factor);
  }
}
...
void av1_setup_butteraugli_rdmult(AV1_COMP *cpi) {
  ...
  av1_set_quantizer(cm, q_cfg->qm_minlevel, q_cfg->qm_maxlevel, q_index,
                    q_cfg->chroma_offset, q_cfg->enable_hdr_deltaq);
